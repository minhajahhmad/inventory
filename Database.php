...
class Database
{
     ...
     */
    public function select($table, $where = array(), $limit = false, $order = false, $where_mode = "AND", $select_fields = '*',$joins=[])
    {
        $this->result = null;
        $this->sql = null;
        $join_this="";
$join_this= implode(" ", $joins);
        ...
        $query = 'SELECT ' . $select_fields . ' FROM `' . $table . "`  {$join_this}";
        if (!empty($where)) {
